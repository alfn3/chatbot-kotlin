...
- Riwayat ditampilkan per item transaksi/produk, bukan hanya dikelompokkan per kontak, dan diurutkan kronologis berdasarkan waktu transaksi.
...
- Rapikan Riwayat screen: hapus judul **Riwayat**, tambahkan margin bottom setelah search bar, tampilkan item per produk secara kronologis, dan gunakan layout kartu dua kolom sesuai format transaksi, harga/saldo, timestamp, dan status.
...
- Daftar riwayat tampil per item produk dan terurut kronologis berdasarkan timestamp.
...
### 8.1 Simulasi UI Chat Pengeluaran

Menambahkan alur chat khusus untuk mencatat pengeluaran operasional konter tanpa keluar dari pola interaksi ChatPOS.

- Kasir dapat mengetik nominal dan keterangan pengeluaran melalui input chat.
- UI menyediakan chips kategori, misalnya **Belanja Stok**, **Transportasi**, **Listrik/Internet**, **Biaya Operasional**, dan **Lainnya**.
- Kasir dapat memilih kategori sebelum atau sesudah mengetik isi pengeluaran.
- Tersedia opsi unggah foto nota sebagai lampiran simulasi.
...
- Kartu pengeluaran menampilkan kategori, nominal, catatan, waktu, dan status pencatatan.
- Alur perlu menyediakan aksi edit, hapus/batalkan, dan konfirmasi agar pengeluaran tidak tercatat secara tidak sengaja.

### 8.2 Screen Uang Laci
...
Fase 1B dikerjakan setelah Definition of Done Fase 1 tercapai. Fokusnya adalah konsistensi pengalaman operasional, validasi input, responsivitas tabel dan kartu, serta state feedback yang mudah dipahami. Integrasi kamera, barcode scanner fisik, akuntansi, penyimpanan permanen, dan sinkronisasi server ditunda ke fase setelahnya.
